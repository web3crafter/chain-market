  ...
  return (
    <footer className="pt-10">
      <div className="relative mx-auto max-w-screen-xl px-4 md:px-8">
        <div className="absolute z-[-1] h-full w-full bg-bottom bg-no-repeat opacity-15 dark:opacity-5 md:bg-right-bottom" />
        {/* <div className="mt-16 flex-1 justify-between space-y-6 sm:flex md:space-y-0">
          {siteConfig.footer.map((item, i) => (
            <ul className="space-y-4 text-muted-foreground" key={i}>
              <h4 className="font-semibold text-gray-800 dark:text-white/90 sm:pb-2">
                {item.label}
              </h4>
              {item.items.map((el, i) => (
                <li key={i}>
                  <Link
                    href={el.href}
                    className="capitalize duration-150 hover:text-gray-800 dark:hover:text-white/90"
                  >
                    {el.name}
                  </Link>
                </li>
